...
<form:form action="/man/template/submit/${editMode}/" commandName="templateForm">
	...
	<c:choose>
		<c:when test="${editMode && templateForm.applySite == 66}">
	...
	<div class="form-group" id="response-version" ${hide}>
		<label class="form-label">模板布局:</label>
		<div class="form-controls">
			<lable class="radio">
				<c:forEach items="${templateSupportedResponseVersions}" var="templateSupportedResponseVersion">
					<input name="responseVersion" type="checkbox" value="${templateSupportedResponseVersion}"
						   <c:forEach items="${templateForm.responseVersion}" var="version">
									<c:if test="${templateSupportedResponseVersion == version}">
										   checked="true"
									</c:if>
							</c:forEach>>${templateSupportedResponseVersion}
				</c:forEach>
			</lable>
			<label class="form-label">
				<a target="_blank" href="http://zh.wikipedia.org/wiki/%E5%93%8D%E5%BA%94%E5%BC%8F%E7%BD%91%E9%A1%B5%E8%AE%BE%E8%AE%A1">何为响应式布局(选填)</a>
			</label>
		</div>
	</div>
    ...
    <div class="form-group" id="USER_NICK">
        ...
        <div class="form-controls">
			...
			（选填，如果留空，默认用户为：<b id="tipsDefaultUserNick">${defaultUserNick}</b>）</span>
        </div>
    </div>
	<div class="form-group">
...
<script type="text/javascript">
	...
	KISSY.use("node",function(S,Node){
        ...
        function changeResponseVersion(siteID){
            if(siteID == 66){
                D.show('#response-version');
            }else{
                D.hide('#response-version');
            }
        }
	});
</script>