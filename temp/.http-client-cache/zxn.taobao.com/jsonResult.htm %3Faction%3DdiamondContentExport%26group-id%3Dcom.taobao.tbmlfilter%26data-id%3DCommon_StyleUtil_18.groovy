...
public class Common_StyleUtil_18 {
    ...
    public static String filter(boolean checkTag, String forbiddenStyle, String value, Map<String, String> attributes) {
        return filter(checkTag, false, forbiddenStyle, value, attributes, null);
    }

    public static String filterForMarket(boolean checkTag, String forbiddenStyle, String value, Map<String, String> attributes, Map<String, String> replaceMap) {
        return filter(checkTag, false, forbiddenStyle, value, attributes, replaceMap)
    }
    ...
    public static String filter(boolean checkTag, boolean checkTbShop, String forbiddenStyle, String value, Map<String, String> attributes) {
        return filter(checkTag, checkTbShop, forbiddenStyle, value, attributes, null);
    }

    public static String filter(boolean checkTag, boolean checkTbShop, String forbiddenStyle, String value, Map<String, String> attributes, Map<String, String> replaceMap) {
        if (!StringUtils.isBlank(value) && attributes != null) {
            ...
            if (checkTag) {
                ...
                value = clearOverideCssII(forbiddenStyle, value, replaceMap);
            }
        }
        return value;
    }
    ...
    public static String filterCssContent(boolean checkTag, boolean checkTbShop, String forbiddenStyle, String value) {
        if (!StringUtils.isBlank(value)) {
            ...
            if (checkTag) {
                ...
                if (checkTbShop)
                    value = value.replaceAll(TB_SHOP, TB_CONTENT);
                value = clearOverideCssII(forbiddenStyle, value, null);
            }
        }
        return value
    }
    ...
    private static String clearOverideCssII(String tbCss, String toBeDealtString, Map<String, String> replaceMap) {
        if (StringUtils.isBlank(toBeDealtString)) return "";
        ...
        if (replaceMap) {
            return clearOverideCssRecursive(toBeDealtString, cachedBlackCssClassPattern, replaceMap)
        }
    ...
    private static String clearOverideCssRecursive(String toBeDealtString, Pattern pattern) {
        def replaceMap = [:] as HashMap<String, String>
        replaceMap.put(PAGE_CONTENT, PAGE_CONTENT_REPLACEMENT)
        return clearOverideCssRecursive(toBeDealtString, pattern, replaceMap)
    }

    /**
     * replaceMap的key是需要保护的，不需要做过滤
     */
    private static String clearOverideCssRecursive(String toBeDealtString, Pattern pattern, Map<String, String> replaceMap) {
        //预替换
        for (String key : replaceMap.keySet()) {
            toBeDealtString = toBeDealtString.replace(key, replaceMap.get(key))
        }
        toBeDealtString = clearOverideCssRecursive_2(toBeDealtString, pattern, replaceMap)
        //替换回来
        for (String key : replaceMap.keySet()) {
            toBeDealtString = toBeDealtString.replace(replaceMap.get(key), key)
        }
        return toBeDealtString;
    }

    private static String clearOverideCssRecursive_2(String toBeDealtString, Pattern pattern, Map<String, String> replaceMap) {
        try {
            if (StringUtils.isBlank(toBeDealtString)) return "";
            ...
            while (m.find()) {
                ...
                String tmp = m.group()
                if (checkBlack(start, tmp) || (checkFront(toBeDealtString, start, tmp) && checkBack(toBeDealtString, end, length))) {
    ...
    private static boolean checkBlack(int start, String group) {
        if (start > 0) {
    ...
    private static boolean checkFront(String toBeDealtString, int start, String group) {
        if (start > 0) {
            if (group != null && (group.charAt(0) == '.'/* || group.charAt(0) == "#"*/)) {
                return true;
            }
