 ...
 */
public class sc_ImageLazyLoadCallback implements AttributeCallback {
    private static final Log logger = LogFactory.getLog(sc_ImageLazyLoadCallback.class)

    private static final String TB_BLACK_URL = "sc-tbml-filter-tbBlackUrl.txt";
    ...
    public String filterValue(TBMLFilter filter, String value, Map<String, String> attributes) {
        try {
            ...
            def blackUrl = TbUrl.getConfigContent()
            if (cacheBlack == null || blackMap == null || !cacheBlack.equals(blackUrl)) {
                cacheBlack = blackUrl
                blackMap = ImageLazyLoadUtil_19.getUrlMap(blackUrl)
            }
            value = ImageLazyLoadUtil_19.filter(blackMap, value);
        } catch (e) {
            logger.error(e)
        }
