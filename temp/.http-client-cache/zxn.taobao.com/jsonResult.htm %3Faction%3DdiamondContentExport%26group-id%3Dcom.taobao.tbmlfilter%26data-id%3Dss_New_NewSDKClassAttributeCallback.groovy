...
public class ss_New_NewSDKClassAttributeCallback implements AttributeCallback {
    private static final Log logger = LogFactory.getLog("TBMLFilter");
    private static final String TB_BLACK_CLASS = "ss-new-sdk-blackClass.txt"
    private static final String TB_WHITE_CLASS = "ss-new-sdk-whiteClass.txt"
    ...
    public String filterValue(TBMLFilter filter, String value, Map<String, String> attributes) {
        try {
            ...
            if(value.contains("cloud-footer")){
                logger.error("ss_New_NewSDKClassAttributeCallback.cloud-footer="+value)
            }
            return Common_ClassUtil_22.filter(blackPattern, whiteList, value, attributes);
        } catch (e) {
            logger.error("ss_New_NewSDKClassAttributeCallback,TBMLFilterException,",e)
        }
        return value
    }
}
